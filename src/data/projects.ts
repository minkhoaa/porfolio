...
export const projects: Project[] = [
  {
    ...
    overview: "A microservice-based AI IELTS preparation platform (.NET 10 + Python FastAPI) built by a two-person team. Engineered an 11-service microservices architecture (10 ASP.NET Core + 1 FastAPI) behind a YARP API gateway, decoupling long-running AI inference (10-40s) from user requests via RabbitMQ + MassTransit. Ran a pilot RAG evaluation (n=5) using counterbalanced methodology and an independent LLM judge, reducing grading MAE from 1.20 to 0.90 against human examiners. Diagnosed critical JSON output failures in reasoning models, stabilizing latency (250s to 33.4s) by migrating grading to Qwen3.8-27b. Deployed local Qwen2.5-1.5B LoRA adapters (r=16, alpha=32, HuggingFace PEFT) and custom PyTorch Wav2Vec2 acoustic scoring on CPU. Benchmarked faster-whisper STT on real 96s speech audio: 2.76s warm inference (RTF 0.03x, 34x faster than real-time) vs 51s cold start, shaping a production model-caching strategy. Sustains 3,027 req/s at 5.63ms p95 on dictionary lookups under 100 concurrent VUs.",
  ...
  },
  {
    ...
    overview: "A full-stack AI social discovery platform featuring an agentic matchmaking engine. Powered by a 3-node LangGraph pipeline (parse intent -> pgvector retrieval -> LLM curation) optimized to $0.12 per 1,000 matches in LLM unit economics. Optimized PostgreSQL pgvector retrieval by replacing sequential scans with HNSW indexing, reducing KNN latency from 1.78ms to 0.667ms (2.7x speedup) and cutting buffer I/O by 70%. Built a ReAct tool-calling agent (Wingman) for proactive engagement with in-memory dispatch scaling linearly with reasoning depth (1.39ms for 1 step -> 6.29ms for 5 steps), capping orchestration overhead under 7ms. Built an in-memory mutex-guarded ranking queue (4.17ms Cosine ranking over 500 candidates, ~240 QPS) eliminating double-match race conditions. Verified resilience with 37 Jest chaos and integration tests (100% pass), confirming SQL fallback on embedding timeouts and 0.007ms fail-open circuit breaking during Gemini 503 outages. Includes hardware-aware speech transcription (Whisper.cpp) with automatic CUDA detection and CPU fallback integrated into a WebRTC signaling gateway.",
