...
export default async function ProjectDetailPage({ params }: Props) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
      <div className="font-mono text-[10px] text-retro-brown mb-8 tracking-wider">
      ...
      <p className="font-pixel text-xs text-retro-muted/60 mb-2 tracking-wider">QUEST {questNumber}</p>

      <div className="flex flex-col sm:flex-row justify-between items-start gap-6">
        <div>
          <h1 className="font-pixel text-2xl sm:text-3xl text-retro-amber leading-relaxed">{project.name}</h1>
        ...
        <div className="flex gap-3 shrink-0">
          <a href={project.github} target="_blank" rel="noopener noreferrer" className="border-2 border-retro-amber px-5 py-2 font-mono text-[10px] text-retro-amber bg-retro-amber/5 hover:bg-retro-amber/15 transition-all duration-300 hover:shadow-[0_0_20px_rgba(167,139,250,0.15)]">VIEW SOURCE</a>
          {project.live && (
            <a href={project.live} target="_blank" rel="noopener noreferrer" className="border-2 border-retro-brown px-5 py-2 font-mono text-[10px] text-retro-muted hover:text-retro-amber hover:border-retro-amber transition-all duration-300">LIVE DEMO</a>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {project.tech.map((t) => (
              <span key={t} className="font-mono text-[11px] text-retro-amber/80 border border-retro-amber/20 bg-retro-amber/5 px-2.5 py-1">{t}</span>
            ))}
          </div>
